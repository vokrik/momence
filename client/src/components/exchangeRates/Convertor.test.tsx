...
describe('Convertor', () => {
  ...
  it('converts by selected rate and rounds up to 2 digits', () => {
    ...
    const result = within(screen.getByTestId("convertor-result")).getByRole('textbox');
  ...
  it('splits currencies by popularity ', () => {
    ...
    expect(within(select).getByTestId('currency-popular-USD')).toBeInTheDocument();
    expect(within(select).getByTestId('currency-other-HRK')).toBeInTheDocument();
  });
  ...
  it('Allows user to change currencies', () => {
    ...
    const select = within(screen.getByTestId('currency-select')).getByRole('combobox');
    ...
    expect((screen.getByRole('option', { name: 'HRK' }) as HTMLOptionElement).selected).toBe(true)
