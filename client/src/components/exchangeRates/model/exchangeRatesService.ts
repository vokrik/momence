...
export const getRates = async (): Promise<Array<ExchangeRateInfo>> =>
  (await fetchRates()).rates;

export const splitByPopularity = (
  allRates: Array<ExchangeRateInfo>,
): PrioritizedRates => {
  const popular = allRates
    .filter((rate) => POPULAR_CODES.includes(rate.code))
    .sort((a, b) => {
      return POPULAR_CODES.indexOf(a.code) - POPULAR_CODES.indexOf(b.code);
    });

  const other = allRates
    .filter((rate: ExchangeRateInfo) => !POPULAR_CODES.includes(rate.code))
    .sort((a, b) => {
