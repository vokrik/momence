...
import {convertToCurrency, ExchangeRateInfo, splitByPopularity} from "./model/exchangeRatesService";
...
export function Convertor({rates}: Props) {
    ...
    const handleCurrencyChange: ChangeEventHandler<HTMLSelectElement> = useCallback((e ) => {
    ...
    return (
        <Grid data-testid="convertor" container spacing={0}>
            <Grid item xs={9}>
                <NumericFormat data-testid="input-czk" allowedDecimalSeparators={[",", "."]} decimalSeparator={','}
            ...
            <Grid item xs={9}>
                <TextField data-testid={"convertor-result"} disabled fullWidth
                           value={state.amountCzk && state.rate && convertToCurrency(state.amountCzk, state.rate).toLocaleString('cs-CZ', {maximumFractionDigits: 2}) || ""}/>
            </Grid>
            <Grid item xs={3}>
                <FormControl
                    variant="outlined"
                    ...
                    >
                        {prioritizedRates.popular.map((rate) => <option data-testid={`currency-popular-${rate.code}`}
                                                                          key={rate.code}
                                                                          value={rate.code}>{rate.code}</option>)}
                        <option disabled>──</option>
                        {prioritizedRates.other.map((rate) => <option data-testid={`currency-other-${rate.code}`}
                                                                        key={rate.code}
                                                                        value={rate.code}>{rate.code}</option>)}
                    </NativeSelect>
                </FormControl>
